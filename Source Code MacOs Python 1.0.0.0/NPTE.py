...
def build():
    ...
    main = [f'python -m nuitka --product-name="{name}" --output-filename={name} --file-version={version} --company-name="{author}" --standalone "{pyfile}"']

    if tkinter_use == "y":
        main.append("--enable-plugin=tk-inter --windows-console-mode=disable")
    ...
    code_parts = [
        "#!/bin/bash",  # Shebang per script bash
        result,
        f'rm -rf {python_file_name}.build',
    ...
    with open(f'{output}/Build.sh', 'w') as my_sh:
        my_sh.write(code)
    
    # Rendi il file eseguibile
    os.chmod(f'{output}/Build.sh', 0o755)

    messagebox.showinfo("Success!", f"The file 'Build.sh' was built successfully at {output}")
